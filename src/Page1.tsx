...
const svgToXY = page1Data.map((item) => {
  ...
  return c.map((child) => {
    const p = child.props;
    const trnasformXY = p.transform ? getTransformXY(p.transform) : { x: null, y: null };
    return {
...
export default function Page1({ isMobile }: Page1Props): ReactElement {
  ...
  const getSvgChild = (child) => child.map((item, i) => {
    ...
    return (
      <Col key={item.nameEn} md={6} xs={24}>
        ...
        <QueueAnim
          ...
          type="bottom"
          component={Link}
          onMouseEnter={() => { onMouseOver(item.nameEn); }}
